...
fn screen_get_text_lossy_returns_string() {
    ...
    assert_eq!(
        vterm.screen_get_text_lossy(&Rect::new(Pos::new(0, 0), Size::new(2, 2))),
        "ab\ncd"
    );
}
...
fn screen_get_text_returns_string() {
    ...
    assert_eq!(
        vterm
            .screen_get_text(&Rect::new(Pos::new(0, 0), Size::new(2, 2)))
            .unwrap(),
        "ab\ncd"
    );
}
...
fn screen_get_text_returns_string_from_part_of_screen() {
    ...
    assert_eq!(
        vterm
            .screen_get_text(&Rect::new(Pos::new(1, 0), Size::new(1, 2)))
            .unwrap(),
        "b\nd"
    );
}
...
fn screen_get_cells_in_rect_returns_cells() {
    ...
    assert_eq!(
        cells.iter().map(|c| c.chars[0]).collect::<Vec<u8>>(),
        b"abcd"
    );
}
